  ...
  <div>
    <el-table
        ...
        <template slot-scope="props">
          <el-form label-position="left"  class="demo-table-expand">
            ...
            <el-form-item label="图片:">
              <span> <img :src="props.row.ps_img" alt="" class="imgSizeShow"></span>
            </el-form-item>
            <el-form-item label="描述:">
              <span class="expandLef">{{ props.row.ps_ft }}</span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="250">
        <template slot-scope="scope">
          <img :src="scope.row.ps_img" alt="" class="imgSize">
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="250">
        <template slot-scope="scope">
          <span style="margin-left: 10px" class="textView">{{ scope.row.ps_ft }}</span>
  ...
  export default {
    ...
    methods: {
      ...
      deleteData(row){
        ...
        console.log(row.ps_id);
        this.axios.get('/api/deleteVuePhyStore.do',{
          params:{
            psId:row.ps_id
          }
        }).then((data)=>{
      ...
      dataOnLoad(index,row){
        ...
        for(let i=0;i<this.tableData.length;i++){
          if(row.ps_id == this.tableData[i].ps_id){    //当前删除的id和总数据中的id比较，相等则删除
    ...
    created() {
      this.axios.get('/api/getVuePhyStore.do').then((data) => {
        console.log(data.data);
        this.tableData = data.data;
        for(let i=0;i<this.tableData.length;i++){
          this.tableData[i].ps_img = phystore.location + this.tableData[i].ps_img;
          // console.log(this.tableData[i].ps_img);
        }
        //存当前页的数据
