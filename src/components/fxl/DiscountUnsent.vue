    ...
    <el-table :data="tableData" height="650" border stripe style="width: 100%">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.diss_id }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="200">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.dis_type }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="180">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.dis_money }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="180">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.dis_usermoney }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="180">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.dis_date }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.$index, scope.row)"  type="primary" plain>发送</el-button>
...
<script>
  export default {
    ...
    methods : {
      ...
      handleEdit(index , row){  // 发送的方法
        console.log("发送");
        let _row = row;
        let date = new Date();
        let dd = date.getFullYear()+"-" + (date.getMonth()+1) + "-" + date.getDate();
        this.$confirm('此操作将发送该优惠券, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'Warning'
        }).then(() => {
          this.updateData({_row,dd});
          this.dataOnLoad(index,_row);
          this.$message({
            type: 'success',
            message: '发送成功!'
      ...
      dataOnLoad(index,row){
        ...
        for(let i=0;i<this.tableData3.length;i++){
          if(row.diss_id == this.tableData3[i].diss_id){    //当前删除的id和总数据中的id比较，相等则删除
      ...
      deleteData(row){
        ...
        console.log(row.diss_id);
        this.axios.post('/fxl/deleteDiscount.do',{
          params:{
            diss_id:row.diss_id
          }
        }).then((data)=>{
      ...
      updateData(obj){
        console.log("调用修改数据的方法");
        this.axios.post('/fxl/updateDiscount.do',{
          params:{
            diss_id:obj._row.diss_id,
            dd:obj.dd
          }
        }).then((data)=>{
          console.log("id:"+data.data)
    ...
    created() {
      // 冯小龙开始介入
      this.axios.post('/fxl/getUnADiscount.do').then((data) => {
