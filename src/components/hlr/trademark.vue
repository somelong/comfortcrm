    ...
    <el-table
        ...
        <template slot-scope="props">
          <el-form label-position="left"  class="demo-table-expand">
            ...
            <el-form-item label="图片:">
              <span> <img :src="props.row.tm_img" alt="" class="imgSizeShow"></span>
            </el-form-item>
            <el-form-item label="描述:">
              <span class="expandLef">{{ props.row.tm_ft }}</span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.tm_zt }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="100">
        <template slot-scope="scope">
          <img :src="scope.row.tm_img" alt="" class="imgSize">
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <span class="textView"   ref="ft" style="margin-left: 10px">{{ scope.row.tm_ft }}</span>
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.tm_tye }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        width="150">
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.tm_morelink }}</span>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      deleteData(row){
        ...
        console.log(row.tm_id);
        this.axios.get('/api/deleteVueTrademark.do',{
          params:{
            tmId:row.tm_id
          }
        }).then((data)=>{
      ...
      dataOnLoad(index,row){
        ...
        for(let i=0;i<this.tableData.length;i++){
          if(row.tm_id == this.tableData[i].tm_id){    //当前删除的id和总数据中的id比较，相等则删除
    ...
    created() {
      this.axios.get('/api/getVueTrademark.do').then((data) => {
        // console.log(data.data);
        this.tableData = data.data;
        for(let i=0;i<this.tableData.length;i++){
          this.tableData[i].tm_img = activity.location + this.tableData[i].tm_img;
